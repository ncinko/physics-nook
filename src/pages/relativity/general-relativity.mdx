...
Outside a nonrotating spherical mass, one important example is the Schwarzschild metric:
...
Once the metric is known, it tells you how to measure tiny intervals and decide what counts as "straight ahead" from one moment to the next. A freely falling object follows a **geodesic**: the straightest possible worldline according to that metric.
...
That is about $6.0 \times 10^{-5}\,\text{s} = 60\,\mu\text{s}$ less than a clock very far from Earth. The effect is tiny here, but it is real: gravity changes the metric, and the metric changes how clocks accumulate time.
