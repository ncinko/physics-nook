...
| `--sim-bg` | a panel holding an interactive, checkpoint, or reference block; recessed one step from the page |
...
In Paper this reads as a warm recessed panel carrying light data surfaces: the vectors demos and the checkpoint sit at `--sim-bg`, the plots inside them at `--surface-plot`, and the hedgehog opener and topics panel at `--surface-elevated`. Two rules follow:
