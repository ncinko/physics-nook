...
`server/realtime.mjs` is the Coaster Park realtime room server. It stays separate from `apps/server` until the coaster prototype is deliberately merged or retired.

## Data Flow

Astro pages render static learning content and hydrate React islands only where interactivity is needed. Site metadata is centralized in `src/data/site.ts` and `src/data/modules.ts`; layouts should read from those sources instead of duplicating SEO defaults.
...
- Put new Astro learning content in `src/pages` and register public metadata in `src/data/modules.ts`.
