...
- Lesson pages: quiet textbook reading surfaces with generous top air, a compact lesson navigator, black-outlined cards, restrained blue active states, and prose in a centered column. The navigator (`ModuleHero`) is one panel holding the module label, the page title at 80% of the heading size, a row of clickable progress ticks with the current lesson in blue and an instant title tooltip on hover, and previous/next links styled like the bottom pager. An icon-only "All lessons" toggle expands the full grid of page cards inline; it stays collapsed by default so a long module never pushes the lesson below the fold. Pages with a topics panel begin their prose with a spaced introduction; the redundant opening title and its divider are visually hidden. In Paper, the topics panel has a lighter cream background around warm ivory page cards; the current page retains its blue border and Here badge.
