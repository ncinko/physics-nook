...
export const onRequestPost = async ({ request, env }: { request: Request; env: Record<string, unknown> }) => {
  ...
  const recent = await db
    .prepare('SELECT COUNT(*) AS count FROM kinematics_goal_rush_runs WHERE ip_hash = ? AND created_at > ?')
  ...
  if (Number(recent?.count ?? 0) >= RUNS_PER_HOUR) {
    return jsonResponse(
      {
        ok: false,
        error: 'Too many goal rush runs. Please wait and try again.',
      },
      { status: 429 },
    );
  }
  ...
  await db
    .prepare(
      `INSERT INTO kinematics_goal_rush_runs (id, ip_hash, user_agent, created_at, expires_at, used_at)
       VALUES (?, ?, ?, ?, ?, NULL)`,
    )
